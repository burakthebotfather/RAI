...
def save_pending_vote(user_id: int, chat_id: int, admin_id: int, message_id: int) -> int:
    conn = get_conn()
    cur = conn.execute(
        "INSERT INTO pending_votes (user_id, chat_id, admin_id, message_id, created_at) VALUES (?, ?, ?, ?, ?)",
        (user_id, chat_id, admin_id, message_id, datetime.utcnow().isoformat())
    )
    conn.commit()
